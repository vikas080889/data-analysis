...
def get_bse_losing_stocks():
    url= "https://api.bseindia.com/BseIndiaAPI/api/MktRGainerLoserData/w?GLtype=gainer&IndxGrp=AllMkt&IndxGrpval=AllMkt&orderby=all"
...
def get_individual_stock_details(stock_list,out_queue):
    try:
        #for i in stock_list:
...
if __name__== "__main__":
    ...
    stock_list = get_bse_losing_stocks()
    dlist =[]

    for count,i in enumerate(stock_list):
        try:
            worker = threading.Thread(target=get_individual_stock_details,args=(i,my_queue))
            worker.start()
            #time.sleep(1.5)
            #worker.my_queue.get()
